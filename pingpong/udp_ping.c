...
/*** TO BE DONE START ***///---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	sent_bytes=nonblocking_write_all(ping_socket, message, msg_size); //(mari) cambiato sizeof con strlen
	if (sent_bytes <0){
		// printf("%zd\n", sent_bytes);
		// printf("ciao9\n");
		fail_errno(strerror(errno));
	}
		// printf("msg_size = %ld", msg_size);
		// printf("msg_size = %ld byte", sizeof (msg_size));
	// printf("ciao5\n");
...
/*** TO BE DONE START ***///---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	// printf("byte arrivati: %zd", recv_bytes);
	// fail_errno(" ");
	recv_errno=errno;
	...
	// 	double time_now = clock_gettime(CLOCK_TYPE, &send_time); 
	// 	double time_end = time_now + UDP_TIMEOUT; 

	// 	recv_bytes = recv(ping_socket, answer_buffer, sizeof(answer_buffer), 0);
		// usleep(RIDUZIONE); //aspetta RIDUZIONE millisecondi
	// 	while(time_now<=time_end && recv_bytes>0) {
	// 		usleep(RIDUZIONE); //aspetta RIDUZIONE millisecondi
	// 		recv_bytes = recv(ping_socket, answer_buffer, sizeof(answer_buffer), 0);
	// 		time_now = clock_gettime(CLOCK_TYPE, &send_time); 
	// 		//ricomincia il ciclo fino a quando o non hai una risposta o non è scaduto il tempo.
	// 	}
		
	// 	if(time_now<=time_end && recv_bytes<0) {
	// 		// recv_time=time_now;
	// 	}
/*** TO BE DONE END ***/	
...
/*** TO BE DONE START ***///---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	if(-1==fcntl(ping_socket, F_SETFL, O_NONBLOCK))
		fail_errno(strerror(errno));
...
/*** TO BE DONE START ***///---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	if(connect (ping_socket, pong_addrinfo->ai_addr, pong_addrinfo->ai_addrlen) == -1) //(virgi) ho modificato il secondo argomento (era server_addrinfo) quindi il tipo era sbagliato
				fail_errno(strerror(errno));
/*** TO BE DONE END ***/
...
/*** TO BE DONE START ***///OK---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	...
	if(connect(ask_socket, server_addrinfo->ai_addr, server_addrinfo->ai_addrlen) < 0) //(virgi) ho modificato il secondo argomento (era server_addrinfo) quindi il tipo era sbagliato
		fail_errno(strerror(errno));
...
/** TO BE DONE START ***/// OK ---------------------------------------------------------------------------------------------------------------------------------------------------------------------
	if(-1==write(ask_socket, request, strlen(request))) /////////////////
		fail_errno(strerror(errno));
